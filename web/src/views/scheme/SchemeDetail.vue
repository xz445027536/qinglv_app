<template>
  <div
      ...
      <div class="left-part">
        <div class="scheme-title">
          <div style="max-width: 550px;">
            <div class="scheme-title-font nowrap-ellipsis" :title="schemeObject.solutionName">
              {{schemeObject.solutionName}}
            </div>
          ...
          <div
            ...
            <div style="width: 100%" class="material-detail-text-container">
              <div class="material-detail-title-container">
                ...
                <div class="material-detail-count">
                  {{`x ${Number(material.count)}`}}
                </div>
              </div>
        ...
        <div class="price-container" style="height:40px;line-height:40px">
          <div style="float:left">
            <span class="price-label-font" style="vertical-align: middle; margin-right: 20px;">方案单价:</span>
            <span class="price-label-font" style="vertical-align: middle; margin-right: 6px;">￥</span>
            <span class="price-number-font" style="vertical-align: middle;">{{schemeObject.cost.toFixed(2)}}</span>
          </div>
          <div style="float:right;font-size:18px;">
            <span>单套方案碳排放量：&nbsp;Kgco2e&nbsp;</span>
            <span style="font-size:28px;color:#2BC7C6;font-weight:bold">{{schemeObject.unitCarbon}}</span>
          </div>
        </div>
        ...
        <div class="delivery-count-container">
          ...
          <div style="margin-top: 32px;">
            <InputNumberAlter
              ...
              :min="0"
              :max="999"
              style="width: 144px; height: 45px;"></InputNumberAlter>
          </div>

          <div class="package-info-container" style="margin-top:44px;margin-bottom:20px">
            <div class="package-needed-font">所需包材套数：</div>
            <div style="">
              <span style="vertical-align: middle; margin-right: 4px;font-size:28px;font-family:Impact;">{{countMultiple}}</span>
          ...
          <div style="width:100%;margin-bottom:60px">
            ...
            <span style="display:inline-block;width:85px;font-size:30px;font-family:Impact;text-align:right">{{(schemeObject.unitCarbon * schemeObject.solQty).toFixed(2)}}</span>
          </div>
          ...
          <div class="material-text-detail-container">
            <div
              ...
              <div style="margin-right: 4px;" class="material-count">x{{countMultiple * Number(material.count)}}</div>
            </div>
          </div>
          ...
          <div class="material-single-container" style="margin-top: 40px; margin-right: -10px;">
              ...
              <Tooltip
                ...
                <div style="width: 30px;">
                  <img style="vertical-align: middle;" width="20" height="20"
                    src="../../assets/scheme/icon_price_hint_white_2x.png" alt="价格提示">
                </div>
              </Tooltip>
            ...
            <div style="flex-shrink: 1;">
              ...
              <span style="margin-right: 8px; vertical-align: middle;font-size:30px;font-family:Impact;">{{(countMultiple * schemeObject.cost).toFixed(2)}}</span>
            </div>
          </div>
    ...
    <div class="scheme-second">
      ...
      <!--<Menu mode="horizontal" :activeName="anchorName" style="margin-left: 30px;">-->
      <!--&lt;!&ndash;<MenuItem style="font-size: 16px;" name="recommend">相关方案</MenuItem>&ndash;&gt;-->
      <!--<MenuItem @click.native="toAnchor('#process')" style="font-size: 16px;" name="process">包装说明</MenuItem>-->
      <!--<MenuItem @click.native="toAnchor('#notice')" v-if="schemeObject.instruSupp.length > 0"-->
      <!--style="font-size: 16px;" name="notice">补充说明-->
      <!--</MenuItem>-->
      <!--<MenuItem @click.native="toAnchor('#comment')" style="font-size: 16px;" name="comment">评论</MenuItem>-->
      <!--</Menu>-->
      <!--</div>-->
      ...
      <div class="anchor-content">
            ...
            <div v-if="imageOrVideo === 'image'" style="position: relative;">
              ...
              <div class="instruction-float-block">
                ...
                <div class="instruction-text">
                  ...
                  <div class="instruction-index-text">{{`(${currentImageIndex + 1}/${processImages.length})`}}</div>
        ...
        <div v-if="schemeObject.instruSupp.length > 0" class="notice-container" id="notice">
          ...
          <div class="notice-area">
            ...
            <div :style="{ 'border-top': noticeIndex === 0 ? '1px solid #eeeeee' : ''}" class="notice-flex-container"
              v-for="(notice, noticeIndex) in noticeInfo" :key="noticeIndex">
              ...
              <div class="notice-content fill-remaining-space">
                <div class="heading-content" v-for="(content, contentIndex) in notice.instruContent"
                  :key="contentIndex">
                  <span class="heading-font" v-if="content.heading">{{content.heading}}</span>
                  <div v-for="(ol, olIndex) in content.orderedList" :key="olIndex">
                    <p :style="{ 'margin-top': olIndex === 0 ? '0' : '12px' }" class="content-font">{{olIndex + 1}}、
                      {{ol}}</p>
                  </div>
                </div>
              </div>
        ...
        <div style="margin-top: 24px;" id="comment">
          <RowTitle title-cn="评论" title-en="Comment"></RowTitle>
          <div class="comment-flex-container">
            <div v-for="comment in schemeObject.comments" :key="comment.commentId"
              class="comment-content-container comment-content-container-margin">
              <img style="flex-shrink: 0;" height="32" width="32" :src="comment.photo" alt="头像">
              <div class="comment-name comment-name-padding">{{comment.name}}</div>
              <div class="fill-remaining-space">
                <div class="comment-content-wrap-container">
                  <div class="comment-content">
                    {{comment.content}}
                  </div>
                </div>

                <div style="margin-top: 20px;" class="icon-text-row-flex-container">
                  <div class="comment-time">{{getTimeDifference(comment.commentTime)}}</div>
                  <a @click="openReplyDialog(comment)">
                    <img style="vertical-align: middle;" width="12" height="12"
                      src="../../assets/scheme/icon_reply_2x.png" alt="回复">
                    <span style="vertical-align: middle; margin-left: 4px;">回复</span>
                  </a>
                  <Poptip
                    confirm
                    title="确定要删除吗？该评论及评论下的回复都会被删除。"
                    @on-ok="deleteComment(comment.commentId)"
                    @on-cancel="cancelDelete">
                    <a
                      v-if="currentUser === comment.empNo || currentUserRole$ === '0'"
                      style="margin-left: 6px;">
                      <img style="vertical-align: middle;" width="12" height="12"
                        src="../../assets/scheme/icon_delete_comment_2x.png" alt="删除">
                      <span style="vertical-align: middle; margin-left: 4px;">删除</span>
                    </a>
                  </Poptip>

                </div>

                <div v-if="comment.reply.length > 0" class="reply-area">
                  <div class="comment-content-container" v-for="(replyData, replyIndex) in comment.reply"
                    :key="replyData.replyFromEmpNo + replyData.replyTime">
                    <div v-if="comment.expand || replyIndex < 3"
                      class="reply-flex-container reply-name-padding fill-remaining-space">
                      <div class="avatar-flex-container">
                        <div>
                          <img style="flex-shrink: 0;" height="32" width="32" :src="replyData.replyFromPhoto" alt="头像">
                        </div>

                        <div class="fill-remaining-space" style="margin: 6px 16px 0 16px;">
                          <div>
                            <span class="reply-name-font">{{replyData.replyFromName}}</span>
                            <span class="reply-name-font" v-if="replyData.repTag === '0'">:</span>
                            <span class="reply-name-font" style="margin-left: 6px;"
                              v-if="replyData.repTag === '1'">回复</span>
                            <span class="reply-name-font" style="margin-left: 6px;" v-if="replyData.repTag === '1'">{{replyData.replyToName}}:</span>
                            <span class="reply-content-font"
                              style="margin-left: 14px;">{{replyData.replyContent}}</span>
                          </div>

                          <div class="icon-text-row-flex-container" style="margin-top: 14px;">
                            <div class="comment-time">{{getTimeDifference(replyData.replyTime)}}</div>
                            <a @click="openSecondReplyDialog(comment, replyData)">
                              <img style="vertical-align: middle;" width="12" height="12"
                                src="../../assets/scheme/icon_reply_2x.png" alt="回复">
                              <span style="vertical-align: middle; margin-left: 4px;">回复</span>
                            </a>
                            <Poptip
                              confirm
                              title="确定要删除该回复吗？"
                              @on-ok="deleteReply(replyData.replyId)"
                              @on-cancel="cancelDelete">
                              <a
                                v-if="currentUser === replyData.replyFromEmpNo || currentUserRole$ === '0'"
                                style="margin-left: 6px;">
                                <img style="vertical-align: middle;" width="12" height="12"
                                  src="../../assets/scheme/icon_delete_comment_2x.png" alt="删除">
                                <span style="vertical-align: middle; margin-left: 4px;">删除</span>
                  ...
                  <div class="expand-tip" v-if="comment.reply.length > 3 && !comment.expand"
                    @click="handleReplyExpandClick(comment)">
                    <span style="vertical-align: middle;">查看全部 {{comment.reply.length}} 条回复</span>
                    <img style="vertical-align: middle; margin-left: 10px;" width="8" height="4"
                      src="../../assets/scheme/icon_reply_expand_2x.png" alt="展开回复">
                  </div>

                  <div class="expand-tip" v-if="comment.reply.length > 3 && comment.expand"
                    @click="handleReplyExpandClick(comment)">
                    <span style="vertical-align: middle;">收起回复</span>
                    <img style="vertical-align: middle; margin-left: 10px;" width="8" height="4"
                      src="../../assets/scheme/icon_reply_fold_2x.png" alt="收起回复">
                  </div>
                </div>
              </div>
            </div>
            ...
            <!--把 Modal 单独拿出来 不要放在 v-for 循环中-->
            <Modal
              v-model="isReplyDialogVisible"
              @on-visible-change="handleCommentVisibleChange"
              :title="'回复' + currentComment.name + ':'">
              <div style="position: relative;">
                <Input ref="commentModalInput" type="textarea" :autosize="{ minRows: 2, maxRows: 4 }" v-model="currentReplyContent"
                  :maxlength="100" placeholder="请输入回复..."/>
                <span :style="{ color: currentReplyContent.length < 100? 'rgba(0, 0, 0, 0.45)' : 'red' }"
                  style="position: absolute; right: 4px; bottom: -20px;">
                  ({{currentReplyContent.length > 100 ? 100 : currentReplyContent.length}}/100)
                </span>
              </div>

              <div slot="footer" class="custom-modal-footer">
                <Button @click="onCommentCancel" style="border: 1px solid #2bc7c6; margin-right: 12px; color: #2bc7c6; background-repeat: no-repeat; background-size: 100% 100%; padding: 4px 19px;">取消</Button>
                <!--<Button type="text" @click="onCommentCancel">取消</Button>-->
                <Button
                  style="border: none; padding: 4px 19px;"
                  :disabled="currentReplyContent.length === 0"
                  :class="{ 'button-background': currentReplyContent.length > 0 }"
                  @click="onCommentOk"
                  type="default">确定
                </Button>
              </div>
            ...
            <Modal
              v-model="isSecondReplyDialogVisible"
              @on-visible-change="handleReplyVisibleChange"
              :title="'回复' + currentReply.replyFromName + ':'">
              <div style="position: relative;">
                <Input ref="replyModalInput" type="textarea" :autosize="{ minRows: 2, maxRows: 4 }"
                  v-model="currentSecondReplyContent" :maxlength="100" placeholder="请输入回复..."/>
                <span
                  :style="{ color: currentSecondReplyContent.length < 100? 'rgba(0, 0, 0, 0.45)' : 'red' }"
                  style="position: absolute; right: 4px; bottom: -20px;">
                  ({{currentSecondReplyContent.length > 100 ? 100 : currentSecondReplyContent.length}}/100)
                </span>
              </div>

              <div slot="footer" class="custom-modal-footer">
                <Button @click="onReplyCancel" style="border: 1px solid #2bc7c6; margin-right: 12px; color: #2bc7c6; background-repeat: no-repeat; background-size: 100% 100%; padding: 4px 19px;">取消</Button>
                <!--<Button type="text" @click="onReplyCancel">取消</Button>-->
                <Button
                  style="border: none; padding: 4px 19px;"
                  :class="{ 'button-background': currentSecondReplyContent.length > 0 }"
                  :disabled="currentSecondReplyContent.length === 0"
                  @click="onReplyOk"
                  type="default">确定
                </Button>
              </div>
          ...
          <div class="my-comment">
            <img style="margin-left: 6%;" src="../../assets/scheme/icon_comment.png" alt="我也说一句">
            <div
              style="margin-left: 12px; margin-right: 40px; font: 14px 'Microsoft YaHei'; color: rgba(0, 0, 0, 0.45);">
              我也说一句
            </div>
            <div style="position: relative;" class="fill-remaining-space">
              <Input v-model="myComment" :maxlength="100" placeholder="请输入评论..."/>
              <span style="position: absolute; top: 7px; right: 7px; color: rgba(0, 0, 0, 0.45);">{{100 - myComment.length >= 0 ? 100 - myComment.length : 0}}</span>
            </div>
            <Button :disabled="myComment.length === 0" class="comment-button"
              :class="{ 'button-background': myComment.length > 0 }" type="primary"
              @click="addComment">评论
            </Button>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      recommendShowList: {
        get: function () {
          if (this.recommendList.length <= 3) {
            ...
            return this.recommendList.slice(this.recommendShowIndex, this.recommendShowIndex + 3)
          }
        }
      },
      ...
      buttonBackground: {
        get: function () {
          return this.myComment.length > 0 ? { 'left-arrow': true } : { 'left-arrow-disabled': true }
        }
      },

      leftArrowClass: {
        get: function () {
          return this.currentImageIndex > 0 ? { 'left-arrow': true } : { 'left-arrow-disabled': true }
        }
      },

      rightArrowClass: {
        get: function () {
          return this.currentImageIndex + 1 < this.processImages.length ? { 'right-arrow': true } : { 'right-arrow-disabled': true }
        }
      }
    ...
    created() {
      ...
      // console.log('config: ', this.config.SHARE_PREFIX)
      // console.log('env: ', process.env.NODE_ENV)
      // console.log('id: ', this.$route.params.id)
      console.log('sharePrefix: ', this.sharePrefix)
    ...
    methods: {
      fetchSchemeDetailData(fetchDataAfterReply = false) {
        getSchemeDetail(this.schemeId).then(res => {
          ...
          this.schemeObject.comments = this.schemeObject.comments.map(comment => {
            comment.reply.map(reply => Object.assign(reply, { secondReplyContent: '' }))
            if (fetchDataAfterReply && comment.commentId === this.currentComment.commentId) {
          ...
          this.processImages = this.schemeObject.prcsImgs.map(item => Object.assign(item, { isSelected: false }))
      ...
      handleCountBlur() {
        if (!this.schemeObject.solQty) {
          ...
          this.saveCarbon = this.schemeObject.unitCarbon * this.schemeObject.solQty;
        }
      },

      // 收藏当前方案
      handleCollectIconClick() {
        collectScheme(this.schemeId).then(res => {
      ...
      handleUnCollectIconClick() {
        unCollectScheme(this.schemeId).then(res => {
      ...
      handleShareIconClick() {
        ...
        if (document.execCommand('copy')) {
          ...
          this.$Message.success({ content: '当前方案链接已复制到剪贴板，可直接粘贴发送分享！', duration: 2.5 })
        }
      ...
      onLeftArrowClick() {
        ...
        const image = this.$refs[this.processImages[this.currentImageIndex].imgId][0]
      ...
      addComment() {
        commentScheme(this.schemeId, this.myComment).then(res => {
          console.log(res)
      ...
      deleteComment(commentId) {
        deleteComment(commentId, 0).then(res => {
          this.$Message.success(res.message)
          this.fetchSchemeDetailData()
        }).catch(err => {
      ...
      deleteReply(replyId) {
        deleteComment(replyId, 1).then(res => {
          this.$Message.success(res.message)
          this.fetchSchemeDetailData()
        }).catch(err => {
          console.log('deleteComment: ', err)
      ...
      onCommentOk() {
        const replyTag = 0
        replyComment(this.currentComment.commentId, this.currentReplyContent, this.currentComment.empNo, replyTag).then(res => {
      ...
      onReplyOk() {
        ...
        replyComment(this.currentComment.commentId, this.currentSecondReplyContent, this.currentReply.replyFromEmpNo, replyTag).then(res => {
