<template>
  <div>
    ...
    <div v-if="classInfo.childCatas.length === 0" style="height: 60vh; width: 100%; margin-top: 12px; background-color: white;">
...
<script>
  ...
  export default {
    ...
    computed: {
      classObj() {
        return this.$store.state.menu.find(clazz => clazz.catId === this.classId)
      }
    },
    data() {
      return {
    ...
    methods: {
      getRouteData(params) {
        ...
        if (this.$store.state.menu) {
          ...
          // if (this.childrenId !== 'all') {
          //   Object.assign(this.classInfo, { childCatas: this.classInfo.childCatas.filter(item =>item.catId === this.childrenId) })
          //   // this.classInfo.childCatas = this.classInfo.childCatas.filter(item =>item.catId === this.childrenId)
