  ...
  <div>
    <div v-if="!showDetail">
      ...
      <el-card class="box-card" v-for="(item,index) in bannerData" :key="item.id">
        ...
        <el-form>
          <el-form-item label="轮播图片:">
            <img :src="item.bannerImg" width="640" height="125"/>
            <el-upload
              class="change-button"
              action=""
              :auto-upload="false"
            ...
            <div style="display: flex;flex-direction: column">
              ...
              <el-upload
                ...
                <div class="uploader-detail-content" v-else @click="detailImageClick(item)">
                  ...
                  <span class="el-upload__tip">上传图片的</span>
                  <span class="el-upload__tip">宽高无限制</span>
                </div>
              </el-upload>
            </div>
          </el-form-item>
      ...
      <el-button type=" primary
            " @click="saveAndUpdate" style="float: right;margin-top: 20px">同步至Web门户
      </el-button>
      <el-dialog
          ...
          <el-form-item label="轮播图片:">
            <el-upload
              ...
              <div class="uploader-content" v-else>
                ...
                <i class="el-icon-plus" style="font-size: 28px"></i>
                <div class="el-upload__tip">2560*500或倍图</div>
              </div>
            </el-upload>
          </el-form-item>
          <el-form-item label="轮播跳转页:">
...
<script>
  import { getBanner, delBanner, saveBanner } from '@/api/pc.js'
  ...
  export default {
    ...
    created() {
      getBanner({ bannerType: '1' }).then(res => {
        if (res.data) {
    ...
    methods: {
      ...
      onNewBannerImageChange(file, fileList) {
        readImageInfo(file).then(res => {
          if (res.width % 2560 === 0 && res.height % 500 === 0) {
            this.setNewImageArr(file)
          } else {
            this.$msgbox({
              title: '图片大小错误',
              message: '图片像素大小为2560*500或其倍图',
              type: 'error'
            })
          }
        })
